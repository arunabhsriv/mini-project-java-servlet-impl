...
////////////////////Sports table CRUD operations
	...
	public int addPlan(Plans plan) throws Exception {
		System.out.println("Inside addplan in dao");
	...
	public int updatePlan(Plans plan) throws Exception {
		...
		String sql = "update plans SET sportId=?,planName=?,fees=?,duration=?  where planId=? ";
		Connection con = DbUtil.getConnection();		
		PreparedStatement ps = con.prepareStatement(sql);
		ps.setInt(1, plan.getSportId());
		ps.setString(2, plan.getPlanName());
	...
	public Plans getPlan(int planId) throws Exception {
		...
		Plans plan = null;
		if(rs.next()) {
			plan = new Plans(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getDouble(4), rs.getInt(5) );
		}
		return plan;
	}
